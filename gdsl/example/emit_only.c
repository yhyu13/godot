...
static gdsl_StringName gdsl_signal_died;
...
/* ---- rule Die by Player ---- */
static void player_die_impl(Player *self) {
	if (!(self->hp <= 0)) {
		return;
	}
	gdsl_emit_signal(self->object, &gdsl_signal_died);
}

static void player_die_call(void *p_method_userdata, GDExtensionClassInstancePtr p_instance, const GDExtensionConstVariantPtr *p_args, GDExtensionInt p_argument_count, GDExtensionVariantPtr r_return, GDExtensionCallError *r_error) {
	player_die_impl((Player *)p_instance);
}

static void player_die_ptr(void *p_method_userdata, GDExtensionClassInstancePtr p_instance, const GDExtensionConstTypePtr *p_args, GDExtensionTypePtr r_ret) {
	player_die_impl((Player *)p_instance);
}
...
static void gdsl_initialize(void *p_userdata, GDExtensionInitializationLevel p_level) {
	...
	gdsl_string_name_new(&gdsl_signal_died, "died", true);
	{
		gdsl_StringName class_name;
		gdsl_StringName parent_name;
		gdsl_string_name_new(&class_name, "Player", false);
		...
		gdsl_classdb_register_extension_class6(gdsl_library, &class_name, &parent_name, &class_info);
		{
			...
			method_info.call_func = player_die_call;
			method_info.ptrcall_func = player_die_ptr;

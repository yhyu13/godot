...
export function scoreLocationMatch(event, cue) {
    const eventPaths = Array.from(new Set(event.context.files_involved.map(normalizeRecallPath)));
    const cuePaths = (Array.isArray(cue.path) ? cue.path : [cue.path]).map(normalizeRecallPath);
...
export function normalizeRecallPath(filePath) {
...
export function scoreStateMatch(event, cue) {
    ...
    // File matching
    if (cue.error?.file) {
        if (event.context.files_involved

...
async function main() {
    ...
    if (relPath === ".wolf" || relPath.startsWith(".wolf/")) {
        process.exit(0);
        return;
    }
 ...
 */
function normalizeErrorSignature(oldStr, newStr) {
    if (!oldStr || !newStr)
        return null;
    const removed = removeWhitespace(oldStr).replace(removeWhitespace(newStr), "");
    if (!removed || removed.length < 8)
        return null;
...
function autoDetectBugFix(wolfDir, absolutePath, relFile, oldStr, newStr) {
    const basename = path.basename(absolutePath);

...
export function resolveProjectPath(projectRoot, filePath) {
    ...
    if (rootIsWindows !== fileIsWindows && path.isAbsolute(filePath))
        return null;
...
export function countSemanticEntries(wolfDir, sessionStarted) {
    ...
    try {
        ...
        const crossedMidnight = hasValidStart && (started.getFullYear() !== now.getFullYear() ||
...
export function wasFileUpdatedSince(filePath, sessionStarted) {
...
export function detectCorrection(text) {
    ...
    const pathRe = new RegExp("(?:in|at|for|of|on|edit(?:ing)?|change(?:ing)?|fix(?:ing)?|read(?:ing)?|inside)\\s+" +
        "([`'\" ]?[\\w./\\\\-]+\\.(" + CODE_EXT_PATTERN + "))", "i");
 ...
 */
export function extractTestFailures(output) {
    ...
    for (const rawLine of output.split(/\r?\n/)) {
        ...
        if (!line)
            continue;
        const isFailure = /^(FAIL|FAILED|✗|✘|×|X)\b/.test(line) ||
